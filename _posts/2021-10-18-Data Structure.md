...
👋 함수 호출 시 전달되는 인자는 항상 매개변수의 사본이 전달됨  
👉 Call-by-Value  
...
　　　　　　　　👋 부수 효과 : 함수 내부의 실행으로 인해 함수 외부 상태가 변경되는 것  
...
　　　　　　　　👋 모듈 : 기능 단위로 분리하여 독립적으로 재활용될 수 있는 소프트웨어 덩어리  
...
　　　　👋 Class의 인스턴스는 힙에 저장되며, 그 힙의 인스턴스 주소 값이 stack에 저장됨  
...
## 오버라이딩(Overriding) ✔️
👉 상속 받았을 때 부모 클래스의 함수를 그대로 사용하지 않고 다른 기능으로 실행하기 위해 자식 클래스에서 동일한 이름으로 함수를 재정의하는 것  
...
　　👋 즉, 데이터 액세스 속도가 빠르다는 장점이 있음  
　　👉 But, 힙은 할당할 때마다 적절한 메모리 공간이 있는지 확인한 후에 할당을 처리하는 동적인 구조  
...
## LinkedList
👉 데이터를 연속적인 메모리 위치에 저장하지 않는 선형 자료구조  
👉 즉, 동적인 크기와 삽입/삭제에 용이  
...
　　👋 2) 제곱법 : key 값을 제곱한 후, key의 비트 값의 중간 부분을 이용하는 방식  
　　👋 3) 폴딩법 : key의 비트 값을 여러 부분으로 나눈 후, 각 부분을 모두 더한 결과 값 or XOR 연산의 결과 값을 사용하는 방식  
　　👋 4) 기수 변환법 : key 값의 진수를 다른 진수로 변환시킨 후, 주소 크기를 벗어난 높은 자릿수를 제거하여 주소 크기에 맞게 사용하는 방법  
　　👋 5) 계수 분석법 : key 값을 이루는 숫자의 분포를 분석하여 비교적 고르게 분포되어 있는 자리 수들을 주소 크기에 맞게 조합하여 사용하는 방법  
...
## 얇은 복사 🆚 깊은 복사 (생성자)
👉 얇은 복사  
　　👉 최소한의 복사를 하여 인스턴스가 메모리에 새로 생성되지 않으며 주소값을 복사하여 같은 메모리를 가리킴  
👉 깊은 복사  
