...
## typealias
v typealias? 기존에 선언되어 있는 유형에 새로운 유형의 별칭을 사용함으로써 코드를 더 읽기 쉽고 이해하기 쉽도록 명확하게 만드는 문법
	typealias ex)
	...
	hello // (title: 1, content: 2, writer: 3)
	hello2 // (title: 4, content: 5, writer: 6)
...
# 1-18
- 타입 캐스팅 : is(타입 확인 및 서브클래스 확인)
- 타입 캐스팅 : as(타입 변환) // 업캐스팅; 서브클래스를 슈퍼클래스로 만드는 것; 항상 성공 // 다운캐스팅; 슈퍼클래스를 서브클래스로 만드는 것; 실패할 수 있기에 as? as! 사용
...
#1 프로토콜의 프로퍼티가 get만 필요로 하는 경우, 모든 종류의 프로퍼티에서 요구사항을 충족시킬 수 있음
...
#2 프로토콜의 프로퍼티가 get과 set 모두 필요로 하는 경우, 상수 저장 프로퍼티와 읽기 전용 연산 프로퍼티가 될 수 없음
...
: 타입 프로퍼티는 static 키워드로만 선언이 가능 // 구현하는 곳에서 class를 선택
