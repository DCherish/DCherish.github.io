...
## Swift Method
👉 Instance Method  
　　👉 클래스를 실체화하여 생성된 인스턴스를 통해 호출할 수 있는 메소드  
👉 Type Method  
...
## Protocol
👉 최소한으로 가져야 할 속성이나 메소드를 정의만 하는 청사진 역할  
...
👉 Delegate  
　　👉 이벤트 처리에 많이 사용되며, 특정 객체에서 발생한 이벤트를 다른 객체에게 전달할 수 있도록 해주는 iOS의 디자인 패턴  
...
　　👉 제 3의 객체를 필요로 하지 X, 확실한 처리 가능, 많은 줄의 코드 필요, 다수의 객체에게 이벤트를 알리고 싶을 경우 비효율적  
👉 Notification  
　　👉 싱글톤 객체인 NotificationCenter를 통해 이벤트 발생 여부를 addObserver한 객체들에게 post하는 방식  
　　👉 주로 이벤트를 1:N로 전달할 때 용이  
　　👉 Delegate 보다 적은 줄의 코드 필요, 다수의 객체에게 동시에 이벤트를 전달이 가능, 제 3의 객체를 반드시 필요로 함, 컴파일 시점에 구독 여부를 확인하는 것이 불가능  
...
👉 값 타입의 프로퍼티는 기본적으로 메소드 내부에서 수정할 수 없는데, 수정이 필요한 경우 Mutating 키워드를 사용하여 수정이 가능해 짐  
👉 이후, 메소드가 종료될 때 변경한 모든 내용을 원래 Struct 혹은 Enum에 기록  
...
👉 UIApplication은 Run Loop를 생성하는 등 실행에 필요한 준비를 마무리  
...
👉 AppDelegate에게 Delegate를 위임하고, Run Loop를 생성하는 역할을 담당  

<br>

## Main Run Loop
👉 유저가 일으키는 이벤트들을 처리하는 프로세스  
👉 UIApplication 객체는 앱이 실행될 때, Main Run Loop를 생성 및 실행하고 이 Loop를 View와 관련된 이벤트나 View 업데이트에 활용함  
...
　　👉 UIApplicationDelegate를 통해 앱의 생명주기 이벤트를 처리함  
...
　　　　👉 빠른 재실행을 위하여 메모리에 적재된 상태지만 실질적으로 동작하고 있지는 않는 상태  
　　　　👉 메모리가 부족할 때 시스템이 강제종료  
　　👉 UISceneDelegate를 통해 Scene 기반 앱의 생명주기 이벤트를 처리함  
　　👉 앱이 Scene을 지원한다면, UIKit은 생명주기 이벤트를 개별적으로 분리하여 전송함  
　　👉 Scene이란, 실행되고 있는 하나의 앱 UI 인스턴스를 뜻하며 각자 개별적으로 자신의 생명주기를 가지고 있음  
　　　　👉 각자의 생명주기를 갖고 있기 때문에 각자 다른 상태에 있을 수 있음  
　　👉 Scene 기반 앱이더라도 추가적인 부가 이벤트를 처리하기 위해서는 UIApplicationDelegate가 필요하며, 이를 통해 처리하도록 함  
　　　　👉 앱 시작 및 종료 관련 이벤트  
👋 [Link][Link1]  
...
　　　　👉 Scene Session이 생성되거나 삭제될 때 AppDelegate에 알리는 두 메소드가 추가됨  
...
👉 @objc는 Swift로 작성한 메서드를 Objective-C로 인식하게 하려고 붙입  
...
　　👉 applicationWillEnterForeground: 애플리케이션이 Active 상태가 되기 직전에, 화면에 보여지기 직전의 시점에 호출.  
　　👉 applicationWillTerminate: 애플리케이션이 종료되기 직전에 호출  

<br>

## UIWindowSceneDelegate
　　👉 UIWindowSceneDelegate를 통해 Scene 기반 앱의 생명주기 이벤트를 처리함  
　　👉   
...
## UIWindow
👉 UIView의 하위 클래스  
👉 사용자 인터페이스에 배경을 제공하고, 중요한 이벤트 처리하는 객체로, 모든 View들의 컨테이너 역할을 함  

<br>

## View
👉 View 객체는 UIView 클래스의 인스턴스  
👉 화면의 직사각형 영역에 콘텐츠를 관리하고 보여주는 역할을 하는 사용자 인터페이스의 기본 구성 요소  
👉 View 객체는 사용자와 애플리케이션이 상호 작용하는 주요 방법이기 때문에 터치, 레이아웃 및 서브 뷰 관리, 이벤트 처리 등의 역할을 수행  
...
👉 NSObject는 Objective-C의 루트 클래스, NSObject를 상속해 Objective-C 런타임에 대한 인터페이스나 기능을 사용할 수 있도록 함  

<br>

## UINavigationController
👉 스택처럼 화면들을 쌓아서 화면 간 이동을 관리하는 컨테이너  
👉 네비게이션 바에서는 뒤로 가기 버튼과 커스터마이징한 버튼을 추가할 수 있고, 옵션으로 툴바 뷰도 제공함  
...
　　　　👉 캡쳐 목록  
　　　　　　👉 클로저 본문에서 하나 이상의 참조 타입을 어떤 참조로 캡쳐 할 지를 정의하는 리스트  
　　　　　　👉 대괄호([]) 키워드를 사용하여 정의  
...
## TableView를 동작 방식과 화면에 Cell을 출력하기 위해 최소한 구현해야 하는 DataSource 메소드를 설명하시오.
...
👉 prepareForReuse는 테이블 뷰나 컬렉션 뷰에서 셀을 재사용할 때 호출되는 메소드입니다. 화면에서 사라진 셀은 리유저블 큐에 저장되고 cellForItemAt 에서 dequeue되었을 때 다시 재사용됩니다. prepareForReuse는 cellForItemAt 이 호출되기 전에 호출되어서 셀의 설정들을 초기화할 수 있도록 도와줍니다.  

<br>

## CollectionViewLayout을 커스텀하게 정의할 때 prepare 메소드가 언제 불리고 어떤 역할을 하는지
👉 콜렉션 뷰가 컨텐츠를 처음 표시하거나 뷰가 변경되어서 레이아웃이 무효화되면 호출됩니다. prepare 메소드가 호출되면 레이아웃 객체에게 레이아웃을 업데이트 하도록 시키게됩니다. 레이아웃이 업데이트 되기 시작하면 콜렉션 뷰가 이 메소드를 호출해서 정의한 레이아웃 객체가 레아이웃을 잡을 수 있게 합니다.  
...
👉 setNeedsLayout은 뷰의 위치와 크기를 업데이트하는 layoutSubviews를 다음 업데이트 사이클에 호출하도록 예약하는 메소드입니다  
👉 setNeedsDisplay는 뷰의 내용을 그리는 draw 메소드를 다음 업데이트 사이클에 호출하도록 예약하는 메소드입니다.  
...
## URLDownloadTask와 URLSessionDataTask의 차이
...
[Link2]: https://blog.naver.com/PostView.naver?blogId=soojin_2604&logNo=222423840595&parentCategoryNo=&categoryNo=&viewDate=&isShowPopularPosts=false&from=postView // MainEventLoop
