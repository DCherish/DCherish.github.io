...
　　👉 private 키워드 사용 시, 컴파일러가 파일 내부에서만 override 여부를 판단하면 되기 때문에 final과 동일한 이유로 성능이 향상  
　　　　👉 final 키워드를 컴파일러가 추론하여 적용할 수 있기 때문  
👉 변수 선언 시 타입을 지정하기  
...
👉 함수가 리턴된 후 함수의 스코프 밖에서 실행시키는 클로져  
...
👉 defer가 호출되기 전에 해당 스코프가 종료(에러반환, 리턴 등)되거나, 리턴 값이 Never(비반환)인 경우에는 defer가 호출되지 X  
