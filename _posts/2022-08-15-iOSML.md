...
👉 하나의 객체에서 여러 객체로 동시에 이벤트를 전달할 때 유용하게 사용할 수 있음  
👉 특정 객체에서 Notification 내부의 Event를 발생시키면, NotificationCenter라는 싱글턴 객체에 등록된 Observer들에 Event가 전달되면서 그에 대한 행동을 취하는 동작 방식  
