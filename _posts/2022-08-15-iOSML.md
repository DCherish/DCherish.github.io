...
👉 함수가 리턴된 후에도 클로져를 실행할 수 있어 비동기 작업을 가능하게 해준다는 특징이 있음 
...
👉 프로토콜을 적용할 객체에, 프로토콜에서 정의했던 프로퍼티와 메소드를 반드시 모두 구현해야 함  
...
　　👉 부모프로토콜과 자식프로토콜 사이가 서로 독립적이기에 필요 없는 메소드나 변수를 갖지 않게 할 수 있음  
...
　　　　👉 약한 참조  
　　　　　　👉 인스턴스가 메모리에서 할당 해제될 때 자동으로 nil로 초기화  
...
## SafeArea
👉 상단의 노치 또는 네비게이션 바나 하단의 홈바 또는 탭바와 같은 이런 시스템에 의해 가려지지 않고 보여지게 되는 영역을 뜻함  
...
## Linear Regression
👉 어떤 변수에 여러 다른 변수들이 주는 영향력을 선형적으로 분석하는 학습 기법  
