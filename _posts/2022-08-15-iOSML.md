...
👉 UIApplication은 @UIApplicationMain 또는 @main 어노테이션이 있는 클래스를 찾아 AppDelegate 객체를 생성하고 Delegate를 위임  
　　👉 @main  
...
　　👉 Scene을 지원하지 않는 경우, 모든 생명주기 관련 이벤트들은 AppDelegate에 전달됨  
　　　　👉 즉, AppDelegate 클래스에서 UI 생명주기를 관리함  
　　👉 AppDelegate는 이름 그대로 앱과 시스템의 연결을 위해 필요한 Delegate 메소드를 담고 있음  
　　👉 AppDelegate 객체는 UIResponder와 UIApplicationDelegate를 상속 및 참조  
　　👉 AppDelegate 객체는 앱의 모든 윈도우를 관리하기 때문에, 앱의 상태 변화는 앱의 모든 UI에 영향을 미침  
👉 iOS 13 이후 동작 (SceneDelegate 있음)  
　　👉 Scene이라는 개념이 생김으로써 동일한 앱 두 개를 실행시킬 수 있게 됨  
　　　　👉 즉, 사용자는 각 앱에 대해 여러 Scene을 만들 수 있음  
　　👉 하나의 앱에 여러 개의 윈도우를 동시에 사용할 수 있게 되었기 떄문에 UI 생명주기를 전담 관리해 줄 클래스가 필요해졌으며, 이를 SceneDelegate라 함  
　　👉 Scene을 지원하는 경우 UIKit은 Scene에 각각 별도의 LifeCycle을 전달  
　　　　👉 여기서의 Scene이란, storyBoard의 Scene과는 개념이 조금 다르니 혼동 x   
　　👉 각 Scene마다 각자의 LifeCycle이 있기 때문에 각각 다른 상태에 있을 수 있음  
...
👉 addTarget의 파라미터  
　　👉 target : 액션을 담당할 객체를 설정하는 파라미터  
　　👉 action : 액션에 대한 행위를 정의한 메소드를 선택하는 파라미터  
...
👉 Objective-C 런타임으로 실행되는 메소드를 지정하기 위해서 사용합니다.

<br>

## 26. TableView를 동작 방식과 화면에 Cell을 출력하기 위해 최소한 구현해야 하는 DataSource 메소드를 설명하시오.
...
## 45. CollectionViewLayout을 커스텀하게 정의할 때 prepare 메소드가 언제 불리고 어떤 역할을 하는지
