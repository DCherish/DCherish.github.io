...
　　　　👉 함수 뒤에 <T>라고 명시하는 이유는 따로 새로운 타입이 아니라 너도 이미 알고 있던, 원래 존재하는 타입이라는 것을 알려주기 위함  
...
Combine : Publisher, Subscriber을 사용해서 비동기 이벤트를 처리할 수 있도록 애플에서 개발한 프레임워크
...
4.
Combine에는 DisposeBag이라는 것이 따로 없으며, 비슷한 기능 동작을 위해서는 store() 메소드와 AnyCancellable을 Set으로 선언하는 방식을 활용하여 구현이 가능함
...
- MVVM in iOS : view에 해당하는 uiview, uiviewcontroller는 각자의 viewmodel을 소유함
viewmodel은 view와 바인딩되어, 데이터와 사용자 액션을 주고 받음
view가 viewmodel을 소유하고 있기 때문에 viewmodel은 view를 몰라도 된다는 장점이 있음

??? viewmodel model


- MVP in iOS : 기존의 MVC에서 많은 역할을 맡고 있던 ViewController의 일을 Presenter에게 나누어 설계한 아키텍쳐
vc에서 유저 액션을 presenter에게 전송함
전송받은 데이터를 presenter에서 model에게 업데이트를 요청함
업데이트 후 model에서 presenter에게 변경사항 알려줌
변경사항을 기반으로 presenter에서 vc로 ui 업데이트를 요청함

model과 vc는 반드시 presenter을 거쳐서 알 수 있으며 직접적으로 서로를 알 수 없음
