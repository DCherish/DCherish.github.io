...
　　👉 @Binding  
　　　　👉 ChildView에서 ParentView의 값을 표시할 때 사용 (ChildView에서 사용됨)  
...
👉 이벤트로 onNext만 받음 (onError는 무시), 즉, UI 이벤트 처리에 좀 더 특화된 것(클래스)  
...
👉 Life Cycle 뿐만이 아닌 대부분의 코드(네트워크 요청 등)가 Controller에 밀집되어 크기가 비대해지고 내부 구조가 복잡해짐  
...
👉 이에 따라, 하나의 Presenter를 통해 유연하게 여러 다른 View를 구성하는 것이 매우 어려워 짐  
