...
---
## Operating System
👉 컴퓨터 시스템의 자원들을 효율적으로 관리하며, 사용자가 편리하고 효과적으로 컴퓨터를 사용할 수 있도록 환경을 제공하는 시스템 소프트웨어  
...
👉 독립적인 메모리 영역을 할당받기에 안정성이 높다는 장점이 있음  
👉 작업량이 많을수록 오버헤드가 발생한다는 단점이 있음  
👋 한 프로세스는 다른 프로세스의 변수나 자료구조에 접근할 수 X  
　　👋 접근하기 위해선 프로세스 간 통신(IPC) 사용  
👋 프로세스는 최소 한 개의 Thread를 가지고 있음  
...
👉 프로세스 간 통신 비용보다 스레드 간 통신 비용이 적다는 장점이 있음  
👉 오류로 인하여 하나의 스레드가 종료 시 전체 스레드가 종료될 수 있다는 단점이 있음  
...
👉 프로세스 간 통신보다 스레드 간 통신 비용이 적기 때문에 멀티 프로세스 대신 멀티 스레드를 사용  
👉 또한, 프로세스를 생성하여 시스템 자원을 할당하는 시스템 콜이 줄어들어 자원을 효율적으로 관리할 수 있기 때문  
...
👋 Race Condition(경쟁 상태) : 두 개 이상의 스레드들이 공유된 자원에 접근하려고 할 때 동기화 메커니즘 없이 접근하려고 하는 상황  

<br>

## 뮤텍스 🆚 세마포어 ✔️
👋 동기화 기법 : 여러 프로세스나 스레드가 동시에 하나의 공유 자원에 접근하는 것을 제어하기 위한 기법  
👉 뮤텍스  
...
　　👉 현재 수행중이 아닌 다른 프로세스 or 스레드가 세마포어를 해제할 수 있음  

<br>

## DeadLock(교착 상태) ✔️
👉 둘 이상의 Task들이 자원을 점유하고 있는 상태에서 서로 다른 Task가 점유하고 있는 자원을 요구하며 무한히 대기하는 상태에 빠진 현상  
👉 교착상태 4가지 조건  
...
👉 교착상태 회피  
　　👉 은행원 알고리즘 사용  
　　　　👉 운영체제가 안전 상태를 유지할 수 있는 요구만을 수락하고 불안전 상태를 초래할 요구는 나중에 안전 상태를 만족할 때까지 계속 거절하는 알고리즘  
...
👉 중기 스케쥴러는 너무 많은 프로세스가 메모리에 동시에 올라가는 것을 조절  
...
## 기아 상태(starvation)
👉 병행 프로세스에서 프로세스가 실행되기 위해 반드시 필요한 자원을 끊임없이 사용하지 못하는 상태  
...
## 세그멘테이션 기법 ✔️
👉 가상 메모리를 이용하여 프로세스를 서로 다른 크기의 논리적인 블록 단위인 세그먼트로 분할하여 메모리에 할당하는 기법  
...
## 가상 메모리
👉 실제 메모리를 보조하여 프로세스 전체가 메모리에 올라오지 않더라도 실행이 가능하도록 하는 기법  
...
👉 이러한 역할을 수행하기 위해 캐시는 CPU가 어떤 데이터를 원할 것인가 어느 정도 예측해야 함  
...
　　👉 Full Associative  
　　　　👉 비어있는 캐시 메모리가 있으면, 비어있는 곳에 마음대로 주소를 저장하는 방식  
　　👉 Set Associative  
　　　　👉 특정 행을 지정하고, 그 행안의 어떤 열이든 비어있는 곳에 마음대로 주소를 저장하는 방식  
...
## Kernel ✔️
👉 하드웨어와 응용 프로그램 사이에서 인터페이스 역할을 하여 응용 프로그램이 하드웨어에서부터 오는 자원을 관리하며 사용할 수 있게 해주는 프로그램  
👉 인터페이스 : 서로 다른 두 시스템이 상호작용할 수 있도록 하는 매개체  
...
## Memory Pool
👉 일정 량의 메모리 블럭을 미리 만들어 pool에 저장하는 기법  
👋 프로그램 요청시 메모리를 할당해주고, 임무 완료시 다시 반환하여 pool에 저장  
👋 불필요하게 Memory를 할당하고 해제하는 일이 사라져서 이와 관련된 비용을 줄여 성능 향상에 도움이 됨  
👋 But, 만들고 사용하지 않아도 계속 메모리에 할당을 해놓으므로 Memory leak 발생할 수 있음  
...
## 커널모드와 유저모드 구분한 이유 (CPU 2가지 모드)
👉 시스템에 중요한 영향을 미치는 연산은 커널 모드에서만 실행 가능하도록 하여 하드웨어 보안을 유지하기 위함  
👋 커널모드 : OS가 호출되어 수행되는 모드  
👋 유저모드 : 어플리케이션 프로그램이 수행되는 모드  
