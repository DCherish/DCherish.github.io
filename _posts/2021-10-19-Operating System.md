...
　　👉 Data : 전역변수, static 변수, 배열, 구조체가 저장됨  
　　　　👉 Data : 초기화 된 데이터가 저장됨  
　　　　👉 Bss : 초기화 되지 않은 데이터가 저장됨  
　　　　👋 구분 이유 : 초기화가 되지 않은 변수는 프로그램이 실행될 때 영역만 잡아주면 되고 그 값을 프로그램에 저장하고 있을 필요가 없으나 초기화가 되는 변수는 그 값도 프로그램에 저장하고 있어야 하기 때문에 두 가지를 구분하여 영역을 잡음  
　　👉 Heap : 동적 할당을 위한 메모리 영역  
　　　　👋 C++ : new, C : malloc()  
　　👉 Stack : 지역변수, 파라미터, 리턴 값 등 잠시 사용되었다가 사라지는 데이터를 저장하는 영역  
...
## Thread pool
👉 병렬 작업이 많아지면 Thread의 개수도 증가  
👉 Thread의 개수가 많아질수록 잦은 Thread 생성과 스케쥴링으로 인하여 애플리케이션의 성능이 저하될 수 있음  
👉 프로세스를 시작할 때, 일정한 수의 Thread를 미리 만들어 pool에 저장하는 것  
👉 작업 큐에 저장되는 작업들을 하나씩 Thread pool에 존재하는 Thread가 처리  

<br>

## 왜 멀티 프로세스 대신 멀티 스레드를 사용?
...
⚠️ But, 전역변수를 함께 사용하여 충돌하는 동기화 문제와 오류로 인하여 하나의 스레드가 종료 시 전체 스레드가 종료될 수 있는 문제를 주의  
...
👉 CPU에 계속 프로세스를 수행시키기 위해 다른 프로세스를 실행시키고 Context Switching, 이 때 걸리는 시간과 메모리를 의미  
...
👉 how?  
　　👉 공유 자원에 접근하는 임계 영역을 **동기화 기법**으로 제어해야 함  
　　👋 뮤텍스, 세마포어, 모니터  
...
## 뮤텍스 🆚 세마포어
👉 공유된 자원의 데이터를 여러 프로세스나 스레드가 접근하는 것을 제어하기 위한 프로세스 동기화 방법  
👉 뮤텍스  
　　👉 권한을 가진 프로세스 or 스레드만 lock or unlock으로 접근할 수 있는 기법  
　　👉 락을 획득한 프로세스가 반드시 그 락을 해제해야 함  
　　👉 동기화 대상이 오직 하나일 때  
👉 세마포어  
...
👉 임계 구역 진입 전까지 루프를 계속 돌고 있기 때문에 busy waiting이 발생  
👉 OS의 스케쥴링 지원을 받지 않으므로 해당 스레드에 대한 Context Switching이 일어나지 X  
👉 짧은 시간 안에 진입할 수 있는 경우 문맥 교환 비용이 들지 않으므로 효율을 높일 수 있음  
👉 반대의 경우에는 다른 스레드에 cpu를 양보하지 않기 때문에 cpu 효율을 떨어뜨림  
👉 Context Switching이 일어나지 않기 때문에 멀티 프로세서 시스템에서만 사용할 수 있음  

<br>

## 모니터
👉 뮤텍스와 조건 변수를 가진 동기화 기법  
👉 최대 하나의 작업만 모니터에 진입 가능  
　　👉 뮤텍스와 개념적으로 유사  
👉 모니터에 선행 작업이 존재할 경우  
　　👉 후행 작업은 대기 상태로 변화  
👉 선행 작업의 signal() 연산이 실행될 경우  
　　👉 선행 작업은 즉시 모니터를 떠남  
　　👉 후행 작업이 자동으로 활성화  
👉 모니터와 개념적으로 유사하지만 뮤텍스는 다른 프로세스 간 동기화할 때 사용할 수 있고, 모니터는 하나의 프로세스 내에서 다른 스레드 동기화에 사용됨  
👉 세마포어는 유용하지만 직접 구현함에 있어 주의를 기울여야 하는 반면, 모니터는 락 획득 및 해제와 관련된 모든 자세한 내용을 다 처리해주기에 간단하게 사용이 가능  
...
👉 교착상태 4가지 조건  
　　👉 상호 배제 : 공유 자원에 대한 접근 제한, 단 하나만 허용  
　　👉 점유 대기 : 공유 자원에 대한 권한을 갖고 있는 Task가 접근 권한을 양보하지 않은 상태에서 다른 자원에 대한 접근 권한을 요구 가능  
　　👉 선취 불가능 : 한 Task가 다른 Task의 접근 권한을 강제로 취소 불가능  
　　👉 순환 대기 : 두 개 이상의 Task가 자원 접근을 기다리는데, 그 관계에 사이클이 존재  
👉 교착상태 방지  
　　👉 4개의 조건 중 하나를 제거하면 됨  
...
## 스케쥴링 단계
👉 장기 스케쥴러에 의해 어떤 프로세스가 **준비 큐**에 삽입될 지를 결정  
...
👉 다른 프로세스가 먼저 도착했어도 가장 CPU 요구량이 적은 것부터 처리  
...
## 기아 상태(starvation)
👉 병행 프로세스에서 프로세스가 실행 되는데에 필수적인 자원을 끊임없이 사용하지 못하는 상태  
...
## 단편화
👉 프로세스들이 메모리에 적재되고 제거되는 일이 반복되다 보면, 프로세스들이 차지하는 메모리 틈 사이에 사용하지 못할 만큼의 작은 자유 공간들이 늘어나게 되는데 이를 단편화라고 함  
　　👉 외부 단편화 : 메모리에 프로세스가 할당될만큼 충분히 공간이 존재하지만 공간이 쪼개져 있어 프로세스를 넣을 수 X  
　　　　👉 해결 기법 : 페이징 기법  
...
## 페이징 기법
👉 가상 메모리를 이용하여 프로세스를 일정한 크기인 페이지로 잘라서 메모리에 적재하는 기법  
👉 하나의 프로세스가 사용하는 메모리 공간이 연속적이어야 한다는 제약을 없애 외부 단편화 문제점을 해결할 수 있음  
👉 But, 그만큼 Mapping 과정 또한 늘어나기 때문에 Trade-off가 발생할 수 있음  
👉 또한, 보통 페이지 단위에 알맞게 꽉 채워 쓰는게 아니므로 내부 단편화 문제는 여전히 존재함  
...
👉 서로 다른 크기의 세그먼트들에 대해 필요 시 메모리에 올리고, 필요없을 경우 내리는 작업을 반복하다보면 외부 단편화가 생기는 문제점이 있음  

<br>

## 가상 메모리
👉 메모리로 실제 존재하지는 않지만 사용자에게 있어 메모리로써 역할을 하는 메모리  
👉 실제 메모리를 보조하여 프로세스 전체가 메모리 내에 올라오지 않더라도 실행이 가능하도록 해주는 역할  

<br>

## Page Fault
👉 지금 실행시켜야 할 Page가 실제 메모리에 올라와 있지 않은 것  

<br>

## Thrashing
👉 프로세스의 처리 시간보다 페이지 교체 시간이 더 많아지는 현상  
👋 프로세스가 원활하게 수행되기 위해 일정 수준 이상의 페이지 프레임을 할당받아야 하는데, 너무 많은 프로세스가 적재되어 프로세스가 프레임을 충분히 할당 받지 못해 Page Fault가 발생  
...
## 캐시의 지역성
👉 캐시 메모리는 CPU의 처리 속도와 주 기억장치의 접근 속도 차이를 줄이기 위해 사용하는 고속 Buffer Memory  
👉 이러한 역할을 수행하기 위해 CPU가 어떤 데이터를 원할 것인가 어느 정도 예측해야 함  
　　👉 캐시의 성능은 작은 용량의 캐시 메모리에 CPU가 이후에 참조할, 쓸모 있는 정보가 어느 정도 들어있느냐에 따라 좌우되기 때문  
👉 적중률을 높이기 위해 데이터 지역성의 원리를 사용  
　　👉 지역성이란 기억 장치 내의 정보를 균일하게 Access 하는 것이 아닌 어느 한 순간에 특정 부분을 집중적으로 참조하는 특성  
　　👉 공간 지역성  
...
## 페이지 교체
👉 프로그램 실행이 계속되면 요구 페이지가 늘어나게 되고 언젠가는 메모리가 가득 참  
...
　　👉 But, 유저 모드에서 커널 모드로 전환이 빈번하게 이뤄지므로 성능이 저하됨  
👉 사용자 수준 스레드  
...
## Memory Pool
👉 필요한 메모리 공간과 개수만큼 pool에 만들어 놓고 필요할 때마다 사용하고 반납하는 메모리 단편화 해결 기법  
👉 메모리 할당과 해제가 잦은 경우 쓰면 효과적  
👉 But, 만들고 사용하지 않아도 계속 할당을 해놓으므로 Memory leak 발생할 수 있음  
