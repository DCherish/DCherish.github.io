...
## 디자인 패턴, 아키텍쳐 패턴 ✔️
👉 소프트웨어 설계에서 공통적인 문제에 대한 표준적인 패턴을 만들어 적용할 수 있게 한 기법  
...
👉 클라이언트와 서버간에 정보를 비동디적으로 전달하기 위해 JavaScript에서 사용하는 하나의 라이브러리  
...
## JRE 🆚 JVM
👉 JRE  
　　👉 JVM 실행 환경을 구현한 것, 프로그램을 동작에 필요한 라이브러리 파일들과 기타 파일들을 가지고 있음  
👉 JVM  
　　👉 자바 프로그램을 컴파일 해서 나온 결과인 바이트 코드(.class)를 실행시켜주는 가상 머신  
...
👉 응용 프로그램에서 운영 체제나 프로그래밍 언어가 제공하는 기능을 제어하며 사용할 수 있게 만든 인터페이스  
