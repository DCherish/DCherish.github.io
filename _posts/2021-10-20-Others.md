...
　　👉 안드로이드 OS로부터 발생하는 각종 event와 정보를 받아와 Handling하는 구성요소  
...
👉 CLEAR_TOP : 액티비티 위에 있는 다른 액티비티를 모두 종료  
...
## Fragment ✔️
👉 부분 화면을 위한 레이아웃  
👉 레이아웃이나 소스 코드를 한 번만 정의하여 재사용할 수 있도록 만든 것  
...
　　👉 2. 등록 ID를 메시지 전송 담당 애플리케이션 서버로 보낸 후 메시지를 기다림  
