...
👉 RDBMS의 테이블은 서로 연관되어 있어 일반 DBMS보다 효율적으로 데이터를 저장, 관리, 구성할 수 있음  
...
👉 속도와 보완 측면에서 PreparedStatement가 우수  
...
## Key ✔️
👉 DB에서 튜플들을 서로 구분할 수 있는 기준이 되는 속성을 뜻함  
...
　　👉 부분 키(Partial Key) : 상위 개체 타입의 키와 결합하여 약한 개체타입의 개별 개체를 고유하게 식별하는 속성  
...
　　👉 검색 시간 복잡도 O(logn), HashTable에 비해 조금 느림  
