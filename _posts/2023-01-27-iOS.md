...
## #selector
👉 런타임 시점에 함수 테이블에서 함수 이름을 참조하여 메서드를 선택하여 실행하는 역할  
...
　　　　　　👉 메모리를 관리하는 방법에 retain/release 키워드 사용하였고, 이를 MRC(Manual Retain Counting)라고 함  
　　　　　　　　👉 retain  
　　　　　　　　　　👉 객체의 레퍼런스 카운트를 증가 시킴  
...
👉 C언어 기반의 저수준 API  
👉 네트워킹 등 무거운 작업을 메인에서 실행시키면 앱이 많이 느려질 수 있음  
　　👉 즉, 무거운 작업은 백그라운드에 맡기고 메인에서는 UI 관련 작업만을 처리하도록 하는 것이 앱에 효율적  
👉 Thread-Safe한 DispatchQueue를 통해 Multi-Threading을 지원  
　　👉 DispatchQueue  
　　　　👉 앱의 Main Thread 혹은 Background Thread의 작업 실행을 관리하는 객체  
　　　　👉 크게 Serial/Concurrent 두 방식이 존재하며, 각 Queue에는 sync/async 존재  
　　　　👉 따라서, 총 4가지(S-s, S-a, C-s, C-a) 형태로 사용 가능  
　　　　　　👉 MainQueue(Serial)  
　　　　　　　　👉 일반적으로 순차적으로 작업을 실행  
　　　　　　　　👉 시스템 작동 시 자동으로 생성  
　　　　　　　　👉 UI 관련 작업은 반드시 이 메인큐에서 동작해야 함  
　　　　　　👉 GlobalQueue(Concurrent)  
　　　　　　　　👉 일반적으로 동시적으로 작업을 실행  
　　　　　　　　👉 메인 큐에서 동작하는 작업에 영향을 주지 않기 위하여 백그라운드에서 동작하는 큐  
　　　　　　　　👉 네트워크 통신과 같이 끝나는 시간을 정확히 알 수 없는 작업을 처리할 때 사용  
　　　　　　👉 sync  
　　　　　　　　👉 큐에 작업을 추가하고 작업이 종료될 때까지 기다린 후 반환  
　　　　　　　　👉 MainQueue는 sync로 작업을 실행하면 데드락이 발생, 따라서 MainQueue는 반드시 async를 사용  
　　　　　　👉 async  
　　　　　　　　👉 큐에 작업을 추가하고 즉시 반환  
